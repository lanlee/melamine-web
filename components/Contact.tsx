...
export default function Contact({ lang }: ContactProps) {
  ...
  return (
    <section id="contact" className="py-28 bg-[#04070f]" ref={ref}>
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-[1fr_1.4fr] gap-16 items-start">
            ...
            <div className="flex flex-col gap-6">
              {[
                ...
                <div key={i} className="flex gap-4 items-start">
                  <div className="w-10 h-10 rounded-xl bg-[#080e1d] border border-[rgba(96,239,255,0.1)] flex items-center justify-center text-lg flex-shrink-0">
                    {item.icon}
                  </div>
                  <div>
                    <div className="text-[10px] font-bold uppercase tracking-widest text-[#7a8eaa] mb-1">
                      {lang === "en" ? item.label_en : item.label_zh}
                    </div>
                    {item.val.split("\n").map((line, j) => (
                      <div key={j} className="text-sm text-white/80">{line}</div>
                    ))}
                  </div>
                </div>
              ))}
          ...
          <motion.div
            ...
            className="bg-[#080e1d] border border-[rgba(96,239,255,0.12)] rounded-2xl p-8 glow-cyan"
          >
            {sent ? (
              <div className="flex flex-col items-center justify-center h-64 gap-4">
                <div className="w-16 h-16 rounded-full bg-[#60efff]/10 border border-[#60efff]/30 flex items-center justify-center text-3xl">
                  ✓
                </div>
                <h3 className="text-xl font-bold text-[#60efff]">
                  {lang === "en" ? "Inquiry Sent!" : "询盘已发送！"}
                </h3>
                <p className="text-[#7a8eaa] text-sm text-center">
                  {lang === "en"
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-[#7a8eaa]">
                    {lang === "en" ? "Message" : "留言"}
                  </label>
                  <textarea
                    rows={4}
                    placeholder={lang === "en" ? "Tell us about your requirements..." : "请告诉我们您的需求..."}
                    className="bg-[#0d1526] border border-[rgba(96,239,255,0.1)] focus:border-[#60efff] rounded-xl px-4 py-3 text-sm text-white/90 placeholder-[#7a8eaa]/40 outline-none transition-colors duration-200 resize-none"
                  />
                </div>
                <button
                  type="submit"
                  className="flex items-center justify-center gap-2 bg-[#60efff] text-[#04070f] font-bold py-4 rounded-xl hover:opacity-90 transition-all duration-200 shadow-[0_0_30px_rgba(96,239,255,0.2)] hover:shadow-[0_0_50px_rgba(96,239,255,0.35)] hover:-translate-y-0.5 text-base"
                >
                ...
                <p className="text-center text-[#7a8eaa] text-xs">
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <input
        ...
        required={required}
        className="bg-[#0d1526] border border-[rgba(96,239,255,0.1)] focus:border-[#60efff] rounded-xl px-4 py-3 text-sm text-white/90 placeholder-[#7a8eaa]/40 outline-none transition-colors duration-200"
      />
    </div>
  );
}
