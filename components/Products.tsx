...
const products = [
  {
    ...
    icon: (
      <svg viewBox="0 0 56 56" fill="none" className="w-14 h-14">
        <polygon points="28,4 52,18 52,38 28,52 4,38 4,18" stroke="#6366f1" strokeWidth="1.5" fill="rgba(99,102,241,0.06)"/>
        <circle cx="28" cy="28" r="8" fill="rgba(99,102,241,0.2)"/>
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 56 56" fill="none" className="w-14 h-14">
        <circle cx="28" cy="28" r="22" stroke="#6366f1" strokeWidth="1.5" fill="rgba(99,102,241,0.06)"/>
        <path d="M17 28L24 35L39 20" stroke="#6366f1" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    ),
          ...
          {products.map((p, i) => (
            <motion.div
              ...
              className={`relative rounded-2xl border p-8 flex flex-col gap-5 card-hover ${
                p.featured
                  ? "border-indigo-200 bg-gradient-to-br from-indigo-50 to-white shadow-[0_8px_32px_rgba(99,102,241,0.12)]"
                  : "border-slate-200 bg-white shadow-sm"
              }`}
            >
              {p.tag_en && (
                <div className="absolute top-5 right-5 bg-[#6366f1] text-white text-[10px] font-black tracking-wider uppercase px-3 py-1 rounded-full shadow-[0_2px_8px_rgba(99,102,241,0.4)]">
              ...
              <div className="flex flex-col gap-2 mt-auto">
                {p.specs.map((s) => (
                  <div key={s.k} className="flex justify-between items-center bg-slate-50 rounded-lg px-4 py-2.5">
                    ...
                    <span className="text-[#6366f1] text-xs font-bold">{s.v}</span>
                  </div>
                ))}
              </div>
              <a
                href="#contact"
                className="mt-2 block text-center border border-slate-200 hover:border-[#6366f1] hover:bg-[#6366f1] hover:text-white text-slate-600 text-sm font-bold py-3 rounded-xl transition-all duration-200"
              >
