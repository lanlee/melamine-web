...
const products = [
  {
    ...
    icon: (
      <svg viewBox="0 0 56 56" fill="none" className="w-14 h-14">
        <polygon points="28,4 52,18 52,38 28,52 4,38 4,18" stroke="#60efff" strokeWidth="1.5" fill="rgba(96,239,255,0.05)"/>
        <circle cx="28" cy="28" r="8" fill="rgba(96,239,255,0.2)"/>
        <circle cx="28" cy="28" r="4" fill="#60efff"/>
      </svg>
    ),
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 56 56" fill="none" className="w-14 h-14">
        <rect x="8" y="8" width="40" height="40" rx="4" stroke="#a78bfa" strokeWidth="1.5" fill="rgba(167,139,250,0.05)"/>
        <rect x="16" y="16" width="24" height="24" rx="2" fill="rgba(167,139,250,0.15)"/>
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 56 56" fill="none" className="w-14 h-14">
        <circle cx="28" cy="28" r="22" stroke="#34d399" strokeWidth="1.5" fill="rgba(52,211,153,0.05)"/>
        <path d="M17 28L24 35L39 20" stroke="#34d399" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    ),
...
export default function Products({ lang }: ProductsProps) {
  ...
  return (
    <section id="products" className="py-28 bg-[#04070f]" ref={ref}>
          ...
          {products.map((p, i) => (
            <motion.div
              ...
              className={`relative rounded-2xl border p-8 flex flex-col gap-5 card-hover ${
                p.featured
                  ? "border-[rgba(96,239,255,0.3)] bg-gradient-to-br from-[#080e1d] to-[rgba(96,239,255,0.02)] glow-cyan"
                  : "border-[rgba(96,239,255,0.08)] bg-[#080e1d]"
              }`}
            >
              {p.tag_en && (
                <div className="absolute top-5 right-5 bg-[#60efff] text-[#04070f] text-[10px] font-black tracking-wider uppercase px-3 py-1 rounded-full">
                  {lang === "en" ? p.tag_en : p.tag_zh}
                </div>
              )}
              {p.icon}
              <div>
                <h3 className="text-xl font-bold mb-2">{lang === "en" ? p.name_en : p.name_zh}</h3>
                <p className="text-[#7a8eaa] text-sm leading-relaxed">{lang === "en" ? p.desc_en : p.desc_zh}</p>
              </div>
              <div className="flex flex-col gap-2 mt-auto">
                {p.specs.map((s) => (
                  <div key={s.k} className="flex justify-between items-center bg-[#0d1526] rounded-lg px-4 py-2.5">
                    <span className="text-[#7a8eaa] text-xs font-medium">{s.k}</span>
                    <span className="text-[#60efff] text-xs font-bold">{s.v}</span>
                  </div>
                ))}
              </div>
              <a
                href="#contact"
                className="mt-2 block text-center border border-[rgba(96,239,255,0.15)] hover:border-[#60efff] hover:bg-[#60efff] hover:text-[#04070f] text-sm font-bold py-3 rounded-xl transition-all duration-200"
              >
