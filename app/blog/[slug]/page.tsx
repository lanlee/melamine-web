...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  post.content = post.content
    .replace(/^##\s+(?:Structured data[^\n]*|JSON-LD[^\n]*)\n[\s\S]*?(?=^##\s|$(?![\s\S]))/gim, "")
    .replace(/```(?:json|jsonld|json-ld)?\s*\n([\s\S]*?)```/gi, (match, body) => body.includes('"@context"') ? "" : match)
    .replace(/^#\s+.+$/gm, "")
    .replace(/^\*\*(?:Author|Date):\*\*.*$/gim, "")
    .replace(/!\[[^\]]*\]\(([^)]+)\)/g, (match, url) => url === heroImage ? "" : match)
    .replace(/^\*Original illustration generated by HeyEmmett\.\*$/gm, "");
  const videoMatch = post.content.match(/\[[^\]]+\]\(https?:\/\/(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([A-Za-z0-9_-]{11})[^)]*\)[^\n]*/);
  const videoId = videoMatch?.[1];
  if (videoMatch) post.content = post.content.replace(videoMatch[0], "");
  post.content = post.content.replace(/^##\s+Relevant video\s*$/gim, "");
